...
Breach types:
    column_removed          — promised column missing from output
    type_changed            — column type changed incompatibly
    null_threshold_exceeded — null % exceeds contract threshold
    row_count_below_minimum — fewer rows than contract requires
    row_count_above_maximum — more rows than contract allows
    unexpected_column       — output has column not in contract (warn only)

Type compatibility uses category mapping: int32/int64 are both "integer",
float32/float64 are both "float". Precision differences are not breaches.
"""
...
def validate_against_contract(
    ...
    for col_name, col_spec in promised_columns.items():
        ...
        # Min/max value checks for numeric columns
