...
describe("fetchApi (via public functions)", () => {
  ...
  it("throws ApiError on non-ok response", async () => {
    ...
    await expect(validatePipeline("bad yaml")).rejects.toMatchObject({
      status: 400,
      message: "invalid",
    });
  });

  it("extracts validation messages from structured error payloads", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({
      ok: false,
      status: 422,
    ...
    await expect(validatePipeline("pipeline:\n")).rejects.toMatchObject({
      status: 422,
      message: "body.yaml_config: Field required",
    });
  });
...
describe("checkHealth", () => {
  ...
  it("throws on failure", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({
      ...
      json: () => Promise.resolve({ message: "backend unavailable" }),
    } as Response);
